...
void pull(int cur,int l,int r){
	seg[cur]=0;
	int mid=(l+r)/2;
	if(tag[cur*2].set!=0)seg[cur]=max(seg[cur],tag[cur*2].set+tag[cur*2].incr);
	else seg[cur]=max(seg[cur],seg[cur*2]+tag[cur*2].incr	);
	
	if(tag[cur*2+1].set!=0)seg[cur]=max(seg[cur],tag[cur*2+1].set+tag[cur*2+1].incr);
	else seg[cur]=max(seg[cur],seg[cur*2+1]+tag[cur*2+1].incr);
}
...
ll que(int l,int r,int cur,int ql,int qr){
	...
	if(l>=ql&&r<=qr){
		if(tag[cur].set!=0)return tag[cur].set+tag[cur].incr;
		else return seg[cur]+tag[cur].incr;
	}
	push(cur,l,r);
	pull(cur,l,r);
	int mid=(l+r)/2;
